...
const DetailMan = props => {
  let { showType, user_vehicleid, getVehicleInfoFn } = props;
  const [detailInfoObj, setDetailInfoObj] = useState(null);
  //驾驶证列表
  const vehicleIdList = arr => {
  ...
  useEffect(() => {
    async function fetchData() {
      let { data } = await getVehicleInfoFn({ user_vehicleid });
    ...
    <>
      {detailInfoObj ? (
        <div className={styles.detail_info_box}>
          ...
          {showType == 'detailVehicle' && (
            <Row>
              <Col span={24}>
                <DescriptionItem
                  title="驾驶证"
                  content={
                    detailInfoObj.vehicle_lic_list_text.split(',').length
                      ? vehicleIdList(
                          detailInfoObj.vehicle_lic_list_text.split(','),
                        )
                      : '暂无图片'
                  }
                />
